...
# Read the motion detection settings from a settings file
with open("settings.txt", "r") as settings_file:
    motion_detection_enabled = settings_file.readline().strip() == "1"
    motion_detection_sensitivity = float(settings_file.readline().strip())
...
# Create the background subtractor for motion detection
...
# A flag to indicate when to refresh streams
        ...
        for i, frame in enumerate(resized_frames):
            if frame is not None:
                ...
                if motion_detection_enabled:
                    ...
                    motion_mask = motion_detector.apply(motion_frame)
                    motion_detected = np.sum(motion_mask) > motion_detection_sensitivity
                    if motion_detected:
                        print(f"Motion detected in stream {i+1} at {time.strftime('%Y-%m-%d %H:%M:%S')}")
